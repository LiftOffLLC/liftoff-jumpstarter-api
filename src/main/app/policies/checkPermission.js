...
export default function checkPermission(permission, keysAndValuePaths = {}) {
  const hasSpecificRole = async(request, reply, next) => {
    ...
    _.each(keysAndValuePaths, (valuePath, key) => {
      additionalParams[key] = (_.hasIn(request, valuePath)) ? _.get(request, valuePath) : valuePath;
    });

    const parameters = _.merge(_.cloneDeep(request.params), additionalParams);
    parameters.userId = request.auth.credentials.userId;

    if (additionalParams.userId && additionalParams.userId !== request.auth.credentials.userId) {
      Logger.info(__filename, 'checkPermission.hasSpecificRole :: impersonated user at play');
      parameters.userId = additionalParams.userId;
      Logger.info(__filename, 'checkPermission.hasSpecificRole :: entry :: parameters :: ', parameters);
      const isAllowed = await rbac.check(UserRole.USER, permission, parameters);
      Logger.info(__filename, 'checkPermission.hasSpecificRole :: exit :: isAllowed :: ', isAllowed);
      return next(null, isAllowed);
    }

    Logger.info(__filename, 'checkPermission.hasSpecificRole :: entry :: parameters :: ', parameters);
    const isAllowed = await rbac.check(request.auth.credentials.scope, permission, parameters);
    Logger.info(__filename, 'checkPermission.hasSpecificRole :: exit :: isAllowed :: ', isAllowed);
    return next(null, isAllowed);
  };
